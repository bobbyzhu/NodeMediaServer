...
function amf0decString(buf) {
    ...
    var buff = new Buffer(sLen);
    buf.copy(buff, 0, 3, 3+sLen);
    return { len: 3 + sLen, value: buff.toString('utf8') }
}
...
function amf0decUString(buf) {
    ...
    var buff = new Buffer(sLen);
    buf.copy(buff, 0, 2, 2+sLen);
    return { len: 2 + sLen, value: buff.toString('utf8') }
}
    ...
    if (rtmpCmdDecode[opt.cmd]) {
        rtmpCmdDecode[opt.cmd].forEach(function (n) {
            if (opt.hasOwnProperty(n))
                ...
                data.write(amf0EncodeOne(opt[n]));
        });
    } else {
